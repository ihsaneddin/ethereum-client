...
namespace Ihsaneddin\Ethereum\Support;
...
class Utils
{
    ...
    {
        if (static::isHex($privateKey) === false) {
            throw new InvalidArgumentException('Invalid private key format.');
        }
        $privateKey = static::stripZero($privateKey);
        if (strlen($privateKey) !== 64) {
            throw new InvalidArgumentException('Invalid private key length.');
        }
        $privateKey = static::secp256k1()->keyFromPrivate($privateKey, 'hex');
    ...
    {
        if (static::isHex($hash) === false) {
            throw new InvalidArgumentException('Invalid hash format.');
        }
        $hash = static::stripZero($hash);
        if (static::isHex($r) === false || static::isHex($s) === false) {
            throw new InvalidArgumentException('Invalid signature format.');
        }
        $r = static::stripZero($r);
        $s = static::stripZero($s);
        if (strlen($r) !== 64 || strlen($s) !== 64) {
            throw new InvalidArgumentException('Invalid signature length.');
        }
        $publicKey = static::secp256k1()->recoverPubKey($hash, [
            'r' => $r,
            's' => $s
        ], $v);
    ...
    public function ecsign(string $privateKey, string $message)
    {
        ...
        $privateKeyLength = strlen(static::stripZero($privateKey));
    ...
    public function hashPersonalMessage(string $message)
    {
        ...
        return static::sha3($prefix . $message);
    }
}
